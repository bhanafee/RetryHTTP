...
/**
 * Predicate to prevent a retry if there is a {@code Retry-After} header that specifies too long a
 * wait interval.
 */
public class LimitRetryAfter implements Predicate<HttpServletResponse> {
  ...
  /**
   * Creates a predicate to limit intervals requested by an HTTP Retry-After header, using an
   * extended parser.
   *
   ...
   * @return false if there is a Retry-After header and the requested duration would exceed the
   *     limit
   */
  @Override
