...
/**
 * Function that extends the wait interval specified by a wrapped {@link IntervalBiFunction} so that
 * it respects any {@code Retry-After} header that was returned in an HTTP response. If a {@code
...
public class HeedRetryAfter implements IntervalBiFunction<HttpServletResponse> {
   ...
   */
  public HeedRetryAfter(
      final IntervalBiFunction<HttpServletResponse> wrapped,
      final Function<HttpServletResponse, Optional<Duration>> parser) {
  ...
  /**
   * Heeds the {@code Retry-After} header with a wait of {@link
   * io.github.resilience4j.retry.RetryConfig#DEFAULT_WAIT_DURATION} if none is required by a
   * header.
   *
  ...
  /**
   * Checks for an HTTP response with a {@code Retry-After} header, and computes the required wait
   * interval.
   *
   * @param t the retry count, which is passed to the wrapped function.
   * @param u the result to evaluate
   * @return the maximum of the wait interval specified by the wrapped function and the interval
   *     indicated by the {@code Retry-After} header.
   */
  @Override
