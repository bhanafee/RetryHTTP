...
public class RetryStatusCodes implements Predicate<HttpServletResponse> {
  ...
  /**
   * Default decisions for non-idempotent retry. Note that index is {@link #OFFSET} from status
   * codes.
   */
  ...
  static {
    ...
    // 425 is due to risk of replay of data during TLS negotiation, expect server to ensure safe
    // retry
   ...
   *
   * @param idempotent whether the service is idempotent. If it is not idempotent, server errors are
   *     not retried unless explicitly allowed by the {@code retry} parameter.
   * @param additional status codes that are expressly allowed for retry. Codes in this list
   *     override defaults.
   * @throws ArrayIndexOutOfBoundsException if any of the additional status codes are out of the
   *     range 100..599
   */
  private RetryStatusCodes(final boolean idempotent, final int... additional) {
    if (additional == null || additional.length == 0) {
      ...
      this.responses =
          Arrays.copyOf(idempotent ? IDEMPOTENT_DEFAULTS : NON_IDEMPOTENT_DEFAULTS, CODES);
   ...
   * @throws ArrayIndexOutOfBoundsException if any of the retry status codes are out of the range
   *     100..599
   */
  ...
  /**
   * Returns a predicate with default decisions for an idempotent service. Idempotent services allow
   * retries of 5xx HTTP status codes except for {@link HttpServletResponse#SC_NOT_IMPLEMENTED} and
  ...
  /**
   * Returns a predicate with default decisions for a non-idempotent service. Non-idempotent
   * services do not allow retries of 5xx HTTP status codes.
   *
   * @param additional status codes that are expressly allowed for retry. Codes in this list
   *     override defaults.
